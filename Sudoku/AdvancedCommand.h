...
/*
* 像save setvalue这种对数独的比较基本的数据操作可以直接用suduku方法，
像auto这样既要候选数清空又要setvalue就得在这个command里写逻辑。
也就是说以后拓展的command的一些复杂操作都是由suduku一些基本数据操作合成的，
这样是我command命令集解决耦合和灵活性的逻辑
*/
class Command
{
public:
...
private:
    Sudoku& sudoku;
    UIInterface& ui;
    PuzzleLoaderInterface& storage;  // 新增文件读写接口
    std::string filename;
public:
...
class BackCommand : public Command //载入上次进度
{
private:   //LoadCommand拥有：模型 界面 文件路径 加载器
    Sudoku& sudoku;
    UIInterface& ui;
